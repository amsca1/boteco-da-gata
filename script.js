...
function renderizarMesas() {
    ...
    if(mesasCadastradas.length === 0) {
        listaUl.innerHTML = '<li class="mesa-vazia">Nenhuma mesa cadastrada</li>';
        return;
    }

    mesasCadastradas.forEach(m => {
        const aberta = m.pedido && m.pedido.length > 0;
        const classeStatus = aberta ? 'mesa-aberta' : '';
        const textoStatus = aberta ? '⚠️ Aberta' : '✅ Livre';

        listaUl.innerHTML += `
            <li class="${classeStatus}" onclick="abrirMesaDireto('${m.id}')" title="Clique para abrir o painel desta mesa">
                <button onclick="event.stopPropagation(); removerCadastroMesa('${m.id}')" class="btn-remover-mesa" title="Remover mesa">✕</button>
                <span class="mesa-nome">${m.nome}</span>
                <span class="mesa-status">${textoStatus}</span>
            </li>
        `;
    });
}
